...
describe("calculatePricing", () => {
  it("returns subscription tier pricing for acreage within range", () => {
    ...
    expect(result.perVisit).toBe(80);
    expect(result.perMonth).toBeCloseTo(80 * (30 / 21), 2);
  });
  ...
  it("returns correct annual pricing for 1.40 acres", () => {
    ...
    expect(result.isCustom).toBe(false);
    expect(result.annualTotal).toBe(3893);
    expect(result.perMonth).toBeCloseTo(3893 / 12, 2);
  });
