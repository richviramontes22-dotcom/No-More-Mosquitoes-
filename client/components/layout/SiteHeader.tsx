...
import { FlagUS, FlagMX } from "@/components/common/FlagIcon";
...
export const SiteHeader = () => {
  ...
  const [fontScheme, setFontScheme] = useState<string>("1");
  ...
  useEffect(() => {
    try {
      const saved = (typeof window !== "undefined" && localStorage.getItem("fontScheme")) || "1";
      setFontScheme(saved);
  ...
  return (
    <header className="relative z-40 border-b border-border/60 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/85">
      ...
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-end gap-4 py-1 text-xs text-muted-foreground">
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-2 rounded-full border border-border/60 bg-background px-3 py-1 text-foreground transition hover:border-border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
              {language === "en" ? <FlagUS /> : <FlagMX />}
              <span className="font-semibold">{language === "en" ? "ENG" : "ESP"}</span>
            </DropdownMenuTrigger>
          ...
          <label htmlFor="font-scheme" className="hidden sm:block">{t("header.fontPreview")}</label>
          <select
            id="font-scheme"
            className="rounded-full border border-border/60 bg-background px-3 py-1 text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            value={fontScheme}
            onChange={(e) => applyFontScheme(e.target.value)}
          >
            <option value="1">1 · Manrope body + Playfair headings (default)</option>
            <option value="2">2 · Inter body + Playfair headings</option>
            <option value="3">3 · Manrope body + Inter headings</option>
            <option value="4">4 · Inter body + Inter headings</option>
            <option value="5">5 · system-ui body + Playfair headings</option>
            <option value="6">6 · system-ui body + Inter headings</option>
            <option value="7">7 · Manrope body + serif headings</option>
            <option value="8">8 · Inter body + serif headings</option>
            <option value="9">9 · serif body + Playfair headings</option>
            <option value="10">10 · system-ui body + system-ui headings</option>
          </select>
        </div>
        <div className="flex items-center justify-between gap-4 py-3">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-6">
            <Link to="/" className="flex items-center gap-3" aria-label="No More Mosquitoes">
              <LogoCutout size={56} className="shrink-0" alt="No More Mosquitoes logo icon" />
            ...
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              ...
              <SheetContent side="right" className="w-full sm:max-w-sm">
                ...
                <SheetTitle className="flex items-center gap-3">
                  <LogoCutout size={40} alt="No More Mosquitoes logo icon" />
                ...
                <div className="mt-6 grid gap-2">
                  <div className="flex gap-2 rounded-xl bg-muted/50 p-2">
                    <button
                      onClick={() => setLanguage("en")}
                      className={`flex-1 flex items-center justify-center gap-1 rounded-lg px-3 py-2 text-sm font-semibold transition ${
                        language === "en"
