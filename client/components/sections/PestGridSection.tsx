...
const PestGridSection = () => {
  ...
  const pestList = t("pests.allPests");

  return (
    <section className="bg-muted/40 py-24">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          eyebrow={t("bugsTreated.eyebrow")}
          title={t("bugsTreated.title")}
          description={t("bugsTreated.description")}
          centered
        />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {Array.isArray(pestList) ? pestList.map((pest) => (
            <div
              key={pest}
