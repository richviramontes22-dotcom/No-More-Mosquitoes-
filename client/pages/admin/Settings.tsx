...
const Settings = () => {
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      toast({ title: "Invite Sent", description: `${name.trim()} has been sent an invite email.` });
      setName("");
      setEmail("");
      setRole("support");
    } catch (err: any) {
  ...
  const handleSaveAll = async () => {
    const success = await saveSettings({ team, flags, integrations });
    if (!success) {
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_400px]">
        ...
        <div className="space-y-8">
          ...
          <div className="sticky top-8 space-y-4">
            <Button className="w-full h-14 rounded-2xl font-bold text-lg shadow-brand" onClick={handleSaveAll} disabled={saving || loading}>
