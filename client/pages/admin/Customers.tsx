  ...
  const fetchCustomers = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .neq("role", "admin")  // Exclude admin profiles from customer view
      ...
      if (data && data.length > 0) {
        ...
        console.log(`[Admin Customers] Query returned empty data`);
        setCustomers([]);
      }
    } catch (err) {
      console.error("[Admin Customers] Exception:", err);
      setCustomers([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const addCustomer = async (c: Omit<Customer, "id">) => {
    try {
      ...
      const res = await fetch("/api/admin/customers/invite", {
        ...
        body: JSON.stringify({ email: c.email, name: c.name, phone: c.phone }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Invite failed");

      toast({ title: "Invite Sent", description: json.message });
      fetchCustomers();
    } catch (err: any) {
...
const CustomerDetailsSheet = ({ customerId }: { customerId: string }) => {
  const [customerData, setCustomerData] = useState<(Customer & { createdAt: string }) | null>(null);
    ...
    const fetchCustomerData = async () => {
      try {
        ...
        const { data: propData, error: propError } = await supabase
          .from("properties")
          .select("id, address, city, state, zip, acreage")
    ...
    <SheetContent className="w-full sm:max-w-3xl p-0 border-none">
      <SheetHeader className="p-8 bg-primary/5 border-b border-border/40">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            <SheetTitle className="text-2xl font-display font-bold">{c.name || "Unknown"}</SheetTitle>
      ...
      <div className="px-8 py-6">
        <Tabs defaultValue="profile" className="w-full">
          <TabsList className="bg-muted/50 p-1 rounded-xl w-full justify-start mb-8">
            ...
            <TabsTrigger value="activity" className="rounded-lg px-6 data-[state=active]:bg-background data-[state=active]:shadow-sm">Activity</TabsTrigger>
            <TabsTrigger value="finance" className="rounded-lg px-6 data-[state=active]:bg-background data-[state=active]:shadow-sm">Finance</TabsTrigger>
          </TabsList>
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="space-y-1">
                  ...
                  <p className="font-medium flex items-center gap-2">
                    <Phone className="h-4 w-4 text-primary" /> {c.phone || "Not provided"}
                  </p>
                </div>
                <div className="space-y-1">
            ...
            {properties.length > 0 ? (
              <div className="rounded-2xl border border-border/40 overflow-hidden bg-card">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/30 border-none">
                      <TableHead className="font-bold text-[10px] uppercase tracking-widest pl-6 py-3">Address</TableHead>
                      <TableHead className="font-bold text-[10px] uppercase tracking-widest py-3">City</TableHead>
                  ...
                  <TableBody>
                    {properties.map((p) => (
                      <TableRow key={p.id} className="border-border/40">
                        <TableCell className="pl-6 py-4 text-sm font-medium">
                          {p.address}
                        </TableCell>
              ...
              <div className="text-center py-12 bg-muted/10 rounded-[32px] border border-dashed border-border/60">
                <p className="text-muted-foreground font-medium italic">No properties found for this customer.</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="activity" className="mt-0 outline-none">
            <div className="grid gap-4">
              <AdminOwnershipNote
                ...
                description="This tab prepares the future unified CRM view. Operational records still live in their dedicated pages."
              >
                <AdminOwnershipBadge kind="future" />
              </AdminOwnershipNote>
              ...
              <AdminEmptyState
                title="Unified activity timeline not built yet"
                description="Appointments, messages, and tickets remain in their current work queues. This placeholder establishes the future customer context model."
              />
            </div>
          </TabsContent>
          ...
          <TabsContent value="finance" className="mt-0 outline-none">
            <div className="grid gap-4">
              <AdminOwnershipNote
                title="Customer billing context foundation"
                description="Billing and marketplace orders still live in Finance. This tab provides a safe navigation bridge for support workflows."
              >
                <AdminOwnershipBadge kind="future" />
              </AdminOwnershipNote>
              <div className="grid gap-3 sm:grid-cols-2">
                <Button variant="outline" className="rounded-xl justify-start" asChild>
                  <a href="/admin/billing">View Billing & Orders</a>
                </Button>
                <Button variant="outline" className="rounded-xl justify-start" asChild>
                  <a href="/admin/revenue">View Revenue Analytics</a>
                </Button>
              </div>
              <AdminEmptyState
                title="Customer-specific finance timeline not built yet"
                description="Payment records and order records are still managed in their current finance views."
              />
            </div>
          </TabsContent>
        </Tabs>
